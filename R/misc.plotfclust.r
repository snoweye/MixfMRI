plotfclust <- function(da, posterior, main = NULL, xlim = NULL, ylim = NULL){
  ...
  image(x, y, da, col = NULL, axes = FALSE, main = main,
        xlim = xlim, ylim = ylim)
  for(i.k in 1:K){
    id.clust <- id
    id.clust[id.clust == TRUE][class != i.k] <- FALSE
    da.clust[!id.clust] <- NA
    da.clust[id.clust] <- posterior[class == i.k, i.k]

    col <- col.image[(i.k - 1) %% length(col.image) + 1]
    col.clust <- my.alpha.col(col)
    image(x, y, da.clust, zlim = c(1 / K, 1), col = col.clust, add = TRUE,
          xlim = xlim, ylim = ylim)
  }
