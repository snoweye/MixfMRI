...
if(!file.exists("new/summary_plotfclust")){
  dir.create("new/summary_plotfclust")
}
...
for(i.case in 1:nrow(ret.summary.org)){
  ...
  file.plot <- paste("./new/summary_plotfclust/",
  ...
  if(i.counter == 1){
    ...
    if(is.na(i.k) || is.na(i.k.org)){
      ...
      plotfclust(da, ret.PARAM.org[[i.k.org]]$posterior, main = "",
    ...
    if(is.na(i.k) || is.na(i.k.org)){
      ...
      plotfclust(da, ret.PARAM.new[[i.k.org]]$posterior, main = "",
                 xlim = i.xlim, ylim = i.ylim)
      text(i.x, i.y.1, labels = paste(tmp.prefix, collapse = "_"),
           cex = i.cex.1, adj = i.adj)
